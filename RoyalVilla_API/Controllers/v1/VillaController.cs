    ...
    public class VillaController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<VillaDTO>>> CreateVilla(VillaCreateDTO villaDTO)
        {
            try
            {
                ...
                var duplicateVilla = await _db.Villa.FirstOrDefaultAsync
                    (u => u.Name.ToLower() == villaDTO.Name.ToLower());

                if (duplicateVilla != null)
                {
                    return Conflict(ApiResponse<object>.Conflict($"A villa with the name '{villaDTO.Name}' already exisits"));
                }
        ...
        public async Task<ActionResult<ApiResponse<VillaDTO>>> UpdateVilla(int id,VillaUpdateDTO villaUpdateDTO)
        {
            try
            {
                ...
                {
                    return BadRequest(ApiResponse<object>.BadRequest("Villa ID in URL does not match with Villa ID in request body"));
                }
                ...
                var duplicateVilla = await _db.Villa.FirstOrDefaultAsync
                    (u => u.Name.ToLower() == villaUpdateDTO.Name.ToLower() && u.Id != id);

                if(duplicateVilla !=null)
                {
                    return Conflict(ApiResponse<object>.Conflict($"A villa with the name '{villaUpdateDTO.Name}' already exisits"));
                }
               ...
               _mapper.Map(villaUpdateDTO, existingVilla);
                ...
                var response = ApiResponse<VillaDTO>.Ok(_mapper.Map<VillaDTO>(villaUpdateDTO),"Villa updated successfully"); 
